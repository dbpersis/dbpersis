...
public class QueryService {
	...
	public Long restQueryCount(String sqlName, Map<String, Object> params)throws Exception {
		String sql = getQueryStatement(sqlName, params);
		List<Map<String, Object>> result = null;
		if (StringUtils.isNotBlank(sql)) {
			result = this.dataSource.jdbcQuery("select count(0) as ct from ("+sql+") u");
		} else {
			result = this.dataSource.jdbcQuery("select count(0) as ct from ("+sql+") u");
		}
		return (Long) result.get(0).get("ct");
	}
	 
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public <T> List<T> query(String sqlName, BeanHandler<T> beanListHandler, Map<String, Object> params, int first,
			int pageSize) throws Exception {
		String sql = getQueryStatement(sqlName, params);
		if(pageSize>0){
			sql = " select * from ( "+sql+" ) u limit "+first +","+pageSize;
		}
		List<T> result = null;
		if (StringUtils.isNotBlank(sql)) {
					...
					for (Object ob : result) {
						...
						for (Field f : fields) {
							...
							if (f.getName().equals(ass.getOwnerProperty())) {
								...
								if (f.getType().toGenericString().contains("interface")) {
									...
									for (Field fs : fields) {
										if (fs.getName().equals(ass.getOwnerParam())) {
											String sqltem = "select * from " + ass.getTargetClass() + " where "
													+ ass.targetProperty + "='" + fs.get(ob) + "'";
											Class<?> doubleClass = Class.forName(f.getType().getName().toString());
											instance = querySingle(sqltem, new BeanHandler(doubleClass), null);
										}
									}
									BeanUtils.setProperty(ob, ass.getOwnerProperty(), instance);
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public <T> T querySingle(String sqlName, BeanHandler<T> beanListHandler, Map<String, Object> params) throws Exception {
		String sql = getQueryStatement(sqlName, params);
		T result = null;
		if (StringUtils.isNotBlank(sql)) {
			result = this.dataSource.querySingle(sql, beanListHandler);
			if (getQueryResultType(sqlName).equals("Map")) {
				return result;
			}
			if (beanListHandler.getType().getSimpleName().equals("Map")) {
			...
			result = this.dataSource.querySingle(sqlName, beanListHandler);
		}
		if(result==null){
			return result;
		}
					...
					if (f.getName().equals(ass.getOwnerProperty())) {
						...
						if (f.getType().toGenericString().contains("interface")) {
							...
							for (Field fs : fields) {
								if (fs.getName().equals(ass.getOwnerParam())) {
									String sqltem = "select * from " + ass.getTargetClass() + " where "
											+ ass.targetProperty + "='" + fs.get(result) + "'";
							...
							BeanUtils.setProperty(result, ass.getOwnerProperty(), list);
						} else {
							Object instance = null;
							for (Field fs : fields) {
								if (fs.getName().equals(ass.getOwnerParam())) {
							...
							BeanUtils.setProperty(result, ass.getOwnerProperty(), instance);
							System.out.println(result);
						}
