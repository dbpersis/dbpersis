...
/**
 * Code style: google java code style.
 * 数据库连接池.
 * 提供数据库操作的模板方法.
 */
public class MyDataSource implements DataSource {

  private static final BlockingQueue<Connection> CONN_POOL = new LinkedBlockingDeque<>();
  ...
  public Connection getConnection() throws SQLException {
    if (!INIT_TAG.get()) {
      throw new RuntimeException("数据库还未初始化好，请待会再试试吧！");
    }
    // 该方法会一直直到连接池中有连接才返回.
    Connection conn;
    try {
      ...
      throw new RuntimeException("Failed to get a connection from pool", e);
    }
  ...
  private static boolean verifyFiled(Field f) {
  ...
  class MyConnection implements Connection {
    ...
    public void close() {
      try {
        ...
        throw new RuntimeException("Failed to close connetion.", e);
      }
      // Release conn
      this.conn = null;
    }
